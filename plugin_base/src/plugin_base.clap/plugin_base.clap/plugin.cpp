...
plugin::timerCallback()
{
  ...
  {
    param_mapping mapping = _engine.desc().global_param_mappings[e.param_index];
...
{ 
  ...
  param_mapping mapping = _engine.desc().global_param_mappings[param_index];
...
{
  auto iter = _engine.desc().param_id_to_global_param_index.find(param_id);
  if (iter == _engine.desc().param_id_to_global_param_index.end())
  {
    assert(false);
    return -1;
  }
...
plugin::paramsFlush(clap_input_events const* in, clap_output_events const* out) noexcept
{
  for (std::uint32_t i = 0; i < in->size(in); i++)
  {
    ...
    auto mapping = _engine.desc().global_param_mappings[index];
    auto const& topo = *_engine.desc().param_at(mapping).topo;
...
plugin::process(clap_process const* process) noexcept
{
  ...
  for (std::uint32_t i = 0; i < process->in_events->size(process->in_events); i++)
  {
    ...
    case CLAP_EVENT_PARAM_VALUE:
    {
      ...
      auto mapping = _engine.desc().global_param_mappings[param_index];
  ...
  for (int e = 0; e < block.output_events.size(); e++)
  {
    ...
    auto mapping = _engine.desc().global_param_mappings[out_event.global_param_index];
