...
graph_engine::process(int module_index, int module_slot, graph_processor processor)
{
  ...
  _last_block = std::make_unique<plugin_block>(
    _engine.make_plugin_block(voice, voice, module_index, module_slot, engine_tuning_mode_off, 0, _host_block->frame_count));
  if(voice == -1)
  ...
  else
  {
    ...
    _last_voice_block = std::make_unique<plugin_voice_block>(
      _engine.make_voice_block(voice, _voice_release_at, id, 1, 0, -1, -1, -1));
