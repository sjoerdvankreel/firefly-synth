...
static param_topo
param_base(
  std::string const& id, std::string const& name, int count, int section, std::string const& default_,
...
make_module(
  ...
  int count, module_scope scope, module_output output)
{
  module_topo result = {};
  result.id = id;
  result.name = name;
...
param_topo
param_toggle(
  std::string const& id, std::string const& name, int count, int section,
  param_direction direction, param_label label, bool default_)
{
  param_topo result(param_base(id, name, count, section, default_? "On": "Off", direction, param_edit::toggle, label, param_rate::block));
  result.min = 0;
  result.max = 1;
...
{
  param_topo result(param_base(id, name, count, section, std::to_string(default_), direction, edit, label, param_rate::block));
  result.min = min;
  result.max = max;
  result.type = param_type::step;
  return result;
...
{
  param_topo result(param_base(id, name, count, section, default_, direction, edit, label, param_rate::block));
...
{
  param_topo result(param_base(id, name, count, section, std::to_string(default_ * 100), direction, edit, label, rate));
  result.min = min;
  result.max = max;
  result.unit = unit? "%": "";
...
{
  param_topo result(param_base(id, name, count, section, std::to_string(default_), direction, edit, label, rate));
  result.min = min;
  result.max = max;
  result.unit = unit;
  result.type = param_type::linear;
