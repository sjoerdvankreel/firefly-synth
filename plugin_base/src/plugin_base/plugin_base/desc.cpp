...
validate_param_topo(module_topo const& module, param_topo const& param)
{
  ...
  assert((param.enabled_indices.size() == 0) == (param.enabled_selector == nullptr));
  for (int i = 0; i < param.enabled_indices.size(); i++)
  {
    assert(!module.params[param.enabled_indices[i]].is_real());
    assert(module.params[param.enabled_indices[i]].slot_count == 1 || module.params[param.enabled_indices[i]].slot_count == param.slot_count);
  }

  assert((param.visibility_indices.size() == 0) == (param.visibility_selector == nullptr));
