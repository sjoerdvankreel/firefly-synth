...
make_audio_routing_audio_params(plugin_state* state, bool global)
{
  ...
  result.matrix_sources = { make_audio_matrix(make_audio_matrix_sources(state->desc().plugin, global), 0).mappings };
  result.matrix_targets = { make_audio_matrix(make_audio_matrix_targets(state->desc().plugin, global), 0).mappings };
  return result;
}
