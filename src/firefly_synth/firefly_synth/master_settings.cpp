...
enum { section_tuning, section_preset, section_smoothing, section_visuals }; 
...
master_settings_topo(int section, gui_position const& pos, bool is_fx, plugin_base::plugin_topo const* plugin)
{
  ...
  std::vector<int> column_distribution = { -153, -225, -427, 1 };
  ...
  auto& tuning_mode = result.params.emplace_back(make_param(
    ...
    make_param_dsp_input(false, param_automate::none), make_domain_item(
      engine_tuning_mode_items(), engine_tuning_mode_items()[engine_tuning_mode_on_note_before_mod].name),
  ...
  auto& midi_smooth = result.params.emplace_back(make_param(
    make_topo_info_basic("{97BAC4E5-B7EE-43AE-AE6D-B9D4A0A3C94F}", "MIDI Smooth", param_midi_smooth, 1),
    make_param_dsp_input(false, param_automate::none), make_domain_linear(1, max_other_smoothing_ms, 50, 0, "Ms"),
    make_param_gui_single(section_smoothing, gui_edit_type::knob, { 0, 0, 1, 1 },
      make_label(gui_label_contents::name, gui_label_align::left, gui_label_justify::near))));
  ...
  auto& bpm_smooth = result.params.emplace_back(make_param(
    make_topo_info_basic("{99B88F8C-6F3B-47E9-932F-D46136252223}", "BPM Smooth", param_tempo_smooth, 1),
    make_param_dsp_input(false, param_automate::none), make_domain_linear(1, max_other_smoothing_ms, 200, 0, "Ms"),
    make_param_gui_single(section_smoothing, gui_edit_type::knob, { 0, 2, 1, 1 },
      make_label(gui_label_contents::name, gui_label_align::left, gui_label_justify::near))));
  ...
  auto& auto_smooth = result.params.emplace_back(make_param(
    make_topo_info_basic("{AF6D2954-3B17-4A32-895B-FB92433761D6}", "Automation Smooth", param_auto_smooth, 1),
    make_param_dsp_input(false, param_automate::none), make_domain_linear(1, max_auto_smoothing_ms, 1, 0, "Ms"),
    make_param_gui_single(section_smoothing, gui_edit_type::knob, { 0, 4, 1, 1 },
      make_label(gui_label_contents::name, gui_label_align::left, gui_label_justify::near))));
  auto_smooth.info.description = "Smoothing automation parameter changes.";
  auto_smooth.info.is_per_instance = true;

  result.sections.emplace_back(make_param_section(section_visuals,
    make_topo_tag_basic("{1E51EDB5-E38A-4401-80F0-789D10C965FA}", "Visuals"),
    make_param_section_gui({ 0, 3 }, { 1, 1 })));
  auto& visuals = result.params.emplace_back(make_param(
    make_topo_info("{D945D383-3BE2-433C-8822-CB0D9E5514D4}", true, "Realtime Visuals", "Visuals", "Visuals", param_visuals, 1),
    make_param_dsp_input(false, param_automate::none), make_domain_item(
      gui_visuals_items(), gui_visuals_items()[gui_visuals_mode_full].name),
    make_param_gui_single(section_visuals, gui_edit_type::autofit_list, { 0, 0, 1, 1 },
      make_label(gui_label_contents::name, gui_label_align::left, gui_label_justify::near))));
  visuals.info.is_per_instance = true;
  visuals.info.description = "Realtime visualization mode";
