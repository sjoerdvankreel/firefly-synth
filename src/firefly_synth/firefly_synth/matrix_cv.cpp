  ...
  routing_matrix<param_topo_mapping> const& targets)
{
  ...
  if(map.module_index == module_vcv_audio_matrix || map.module_index == module_vcv_cv_matrix)
    relevant_modules.insert(relevant_modules.end(), { 
    module_vcv_cv_matrix, module_arpeggiator, module_voice_note, module_voice_on_note, module_vlfo, module_env });
  ...
  std::vector<module_topo const*> const& targets)
{
  ...
  if (!global) result.dependent_custom_outputs_module_topo_indices = {
    module_glfo, module_arpeggiator, module_env, module_vlfo, module_voice_note, module_voice_on_note };
