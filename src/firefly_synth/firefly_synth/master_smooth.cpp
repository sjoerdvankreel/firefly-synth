...
extern int const master_smooth_param_auto_smooth = param_auto_smooth;
extern int const master_smooth_param_midi_smooth = param_midi_smooth;
extern int const master_smooth_param_tempo_smooth = param_tempo_smooth;
...
module_topo
master_smooth_topo(int section, gui_position const& pos)
{
  ...
  result.sections.emplace_back(make_param_section(section_main,
    make_topo_tag_basic("{D02F55AF-1DC8-48F0-B12A-43B47AD6E392}", "Master Smoothing"), section_gui));
  auto& midi_smooth = result.params.emplace_back(make_param(
    make_topo_info("{887D373C-D978-48F7-A9E7-70C03A58492A}", true, "MIDI Smoothing", "MIDI Smt", "MIDI Smt", param_midi_smooth, 1),
  ...
  auto& bpm_smooth = result.params.emplace_back(make_param(
    make_topo_info("{AA564CE1-4F1E-44F5-89D9-130F17F4185C}", true, "BPM Smoothing", "BPM Smt", "BPM Smt", param_tempo_smooth, 1),
  ...
  auto& auto_smooth = result.params.emplace_back(make_param(
    make_topo_info("{852632AB-EF17-47DB-8C5A-3DB32BA78571}", true, "Automation Smoothing", "AutoSmt", "Auto Smt", param_auto_smooth, 1),
