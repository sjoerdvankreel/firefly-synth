...
enum { 
  module_section_hidden, module_section_master_cv, module_section_voice_cv,
...
{
  if (global)
    return { &topo->modules[module_gfx], &topo->modules[module_master_audio_out] };
  else
    return { &topo->modules[module_vfx], &topo->modules[module_voice_audio_out] };
}
...
synth_topo()
{
  ...
  result->gui.module_sections[module_section_voice_cv] = make_module_section_gui(
    "{FB435C64-8349-4F0F-84FC-FFC82002D69F}", module_section_voice_cv, { 2, 0, 1, 3 }, { 1, 1 });
  result->gui.module_sections[module_section_voice_master_audio] = make_module_section_gui(
    "{8FDAEB21-8876-4A90-A8E1-95A96FB98FD8}", module_section_voice_master_audio, { 7, 0, 1, 3 }, { { 1 }, { 1, 1 } });
  ...
  result->modules[module_voice_cv] = voice_cv_topo(module_section_voice_cv, master_colors, { 0, 0 });
  ...
  result->modules[module_vcv_matrix] = cv_matrix_topo(module_section_cv_matrix, cv_colors, { 0, 0 }, false,
    make_cv_matrix_sources(result.get(), false, { module_voice_on_note }),
